...
![](https://latex.codecogs.com/png.latex?AMI=\frac{1}{n}%20\log_{ }{\frac{p(W)}{p(c_{1})%20\cdots%20p(c_{n})}})
...
* 词语的score计算通过内部的get_scores函数实现，score公式如下：
\begin{equation}
AMI=\frac{1}{n} \log_{}{\frac{p(W)}{p(c_{1}) \cdots p(c_{n})}},
\end{equation}

\begin{equation}
L(W)=\log_{}{\frac{LE \cdot e^{RE}+RE \cdot e^{LE}}{|LE-RE|}},
\end{equation}
